 ...
 * node is within limit it will set a path to it.  If not it will find a shortest path 
 ...
 */
import java.util.*;
public class shortestPlayer extends Player
{
    private ArrayList<Integer> oneBefore;
    public shortestPlayer(UndirectedGraph graph,int limit, int beginning, ArrayList<Integer> end){
        super(graph,limit,beginning,end);
        oneBefore = new ArrayList<Integer>();
        oneBefore.add(0);
        playerName = "Shortest algorithm";
    }
     ...
     * will choose a path to the shortest distance of the neighbors of the current index
     */
            ...
            if(steps ==0){
                ...
                if(checkForExitD()){
                    ...
                    if(steps - value <=0 && whereTo == exit){
                        System.out.println("shortest Algorithm Wins!");
                        System.exit(0);
                    }
                    ...
                    if(neighbors(whereAt).size()==1){
                        ...
                        oneBefore.remove(0);
                        int whichNeihbor = shortest(withOutOneICameFrom);
    ...
    public int shortest(ArrayList<Integer> find){
        int shortest = (int)(Integer.MAX_VALUE);
        for(int i =0;i<find.size();i++){
            if(shortest > weight(whereAt,find.get(i)) && weight(whereAt,find.get(i)) <limit){
                shortest = find.get(i);
            }
        }
        return shortest;
